...
export default async function InvitePage({
  ...
  // Find the event via the invite token
  ...
  return (
    <div className="mx-auto w-full max-w-lg">
      {isSubmitted && (
        <div className="animate-fade-in-up mb-6 rounded-lg border border-violet-500/30 bg-violet-500/10 p-4 text-sm text-violet-300">
        ...
        {event.rsvps.length > 0 && (
          <div className="mt-6 border-t border-white/10 pt-4">
            <p className="text-xs text-white/40 text-center">
              ...
              {event.rsvps.filter((r) => r.status === "not_going").length} not going
            </p>
          </div>
        )}
      </div>
